...
/**
  * 第六步：训练多个模型，取其中最好，即取RMSE(均方根误差)值最小的模型
  *
...
object ModelTraining extends App {
  ...
  def main(args: Array[String]) {
    ...
    val trainingData = hc.sql("select * from trainingData") // 训练集，总数据集的60%
    val testData = hc.sql("select * from testData") // 测试集，总数据集的40%

    // 训练集，转为Rating格式
    val ratingRDD = hc.sql("select * from trainingData").rdd.map(x => Rating(x.getInt(0), x.getInt(1), x.getDouble(2)))
    val training2 = ratingRDD.map { // 用于计算模型的RMSE(均方根误差)
    ...
    val test2 = testRDD.map { // 用于计算模型的RMSE
    ...
    for (l <- lambda; i <- iteration) {
      val model = ALS.train(ratingRDD, rank, i, l) //训练模型
