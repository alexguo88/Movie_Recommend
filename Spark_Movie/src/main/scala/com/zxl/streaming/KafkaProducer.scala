...
object KafkaProducer extends App {
  ...
  def main(args: Array[String]) {
    ...
    prop.put("bootstrap.servers", "spark1:9092") // 指定kafka的 ip地址:端口号
    //    prop.put("acks", "all") // 配置可以设定发送消息后是否需要Broker端返回确认，有"0"，"1"，"all"
    //    prop.put("retries", "0")
    //    prop.put("batch.size", "16384")
    //    prop.put("linger.ms", "1")
    //    prop.put("buffer.memory", "33554432")
    prop.put("key.serializer", "org.apache.kafka.common.serialization.StringSerializer") // 设定ProducerRecord发送的key值为String类型
    prop.put("value.serializer", "org.apache.kafka.common.serialization.StringSerializer") // 设定ProducerRecord发送的value值为String类型
    val producer = new KafkaProducer[String, String](prop)


    // testDF
    val testDF = hc.sql("select * from testData limit 10000") // 如果数据不加 limit限制，会出现OOM错误
    val topic = "movie"
    val testData = testDF.map(x => (topic, x.getInt(0).toString() + "," + x.getInt(1).toString + "," + x.getDouble(2).toString()))
    val messages = testData.toLocalIterator // 如果服务器内存不够，会出现OOM错误
